...
from src.learning.memory_store import ExperienceMemory
from src.learning.critic_agent import CriticAgent
from src.learning.experience_retriever import retrieve_past_experiences
...
async def collect_primary_data(signal):

    symbol = signal.get("underlyingSymbol", "")
    exchange = signal.get("exch", "")
    date = signal.get("lttDate", "")

    query = f"{symbol} stock on {exchange} around {date}"
...
async def make_decision(signal):
    ...
    past_experiences = retrieve_past_experiences(signal)
    print(f"Past Experiences: {past_experiences}")
    print("================================== Up Data Past Experiences =====================================")
